...
// It's good practice to name page components with "Page" at the end
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (userRole === 'employee') {
      router.push('/employee/dashboard'); // <-- Use router.push for navigation
    }
  }, [userRole, router]);
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                {userRole === 'hr' && (
                  <>
                    <Link href="/jobs/new" className="block w-full text-left p-3 rounded-lg hover:bg-secondary transition-colors">
                      <p className="font-medium">Post New Job</p>
                      <p className="text-sm text-muted-foreground">Create a new job listing</p>
                    </Link>
                    <Link href="/applications" className="block w-full text-left p-3 rounded-lg hover:bg-secondary transition-colors">
                ...
                {userRole === 'interviewer' && (
                  <>
                    <Link href="/interviews/schedule" className="block w-full text-left p-3 rounded-lg hover:bg-secondary transition-colors">
                      <p className="font-medium">View Schedule</p>
                      <p className="text-sm text-muted-foreground">Today's interviews</p>
                    </Link>
                    <Link href="/reviews/pending" className="block w-full text-left p-3 rounded-lg hover:bg-secondary transition-colors">
                ...
                {userRole === 'candidate' && (
                  <>
                    <Link href="/jobs" className="block w-full text-left p-3 rounded-lg hover:bg-secondary transition-colors">
                      <p className="font-medium">Browse Jobs</p>
                      <p className="text-sm text-muted-foreground">Find new opportunities</p>
                    </Link>
                    <Link href="/profile" className="block w-full text-left p-3 rounded-lg hover:bg-secondary transition-colors">
